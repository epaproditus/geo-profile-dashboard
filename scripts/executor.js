...
// This is a clean direct executor script that avoids duplication issues
...
function logToFile(message) {
  try {
    ...
    // Attempt to log to file if we have permission
    try {
...
async function sendCronitorTelemetry({ status = 'success', message = '', metrics = {} }) {
  try {
    ...
    logToFile(`Sending telemetry to Cronitor: ${status} - ${message}`);
    
    // Send the telemetry ping
    const response = await fetch(telemetryUrl);
    
    if (!response.ok) {
      logToFile(`Failed to send Cronitor telemetry: ${await response.text()}`);
    } else {
      logToFile('Cronitor telemetry sent successfully');
    }
  } catch (err) {
    logToFile(`Error sending Cronitor telemetry: ${err.message}`);
  }
}
...
async function fetchAllDevices(apiKey) {
  try {
    logToFile('Fetching all devices from SimpleMDM');
    ...
    logToFile(`Successfully fetched ${data.data.length} devices`);
    
    return { data: data.data };
  } catch (error) {
    logToFile(`Error fetching devices: ${error.message}`);
    return { data: [], error };
  }
}
...
async function fetchFilteredDevices(filter, apiKey) {
  logToFile(`Applying device filter: ${JSON.stringify(filter)}`);
  ...
  if (filter.groupIds && filter.groupIds.length > 0) {
    ...
    logToFile(`Group ID filtering not implemented yet. Requested groups: ${filter.groupIds.join(', ')}`);
  }
  
  if (filter.nameContains) {
    logToFile(`Filtering devices by name containing: ${filter.nameContains}`);
    filteredDevices = filteredDevices.filter(device => 
      device.attributes.name.toLowerCase().includes(filter.nameContains.toLowerCase())
    );
  }
  
  logToFile(`Filter applied. ${filteredDevices.length} devices matched.`);
...
async function pushProfileToDevice(profileId, deviceId, apiKey) {
  try {
    logToFile(`Pushing profile ${profileId} to device ${deviceId}`);
    ...
    logToFile(`Successfully pushed profile ${profileId} to device ${deviceId}`);
    return { success: true };
  } catch (error) {
    logToFile(`Error pushing profile ${profileId} to device ${deviceId}: ${error.message}`);
    return { success: false, error: error.message };
  }
}

/**
 * Main function to execute all schedules
 */
async function executeSchedules() {
  ...
  try {
    logToFile('Starting schedule execution');
    ...
    logToFile('Initializing Supabase client');
    ...
    logToFile(`Searching for schedules between ${pastWindow.toISOString()} and ${now.toISOString()}`);
    ...
    if (!schedulesToExecute || schedulesToExecute.length === 0) {
      logToFile('No schedules to execute at this time');
    ...
    const results = await Promise.all(schedulesToExecute.map(async (schedule) => {
      try {
        logToFile(`Processing schedule ${schedule.id}: ${schedule.name || 'Unnamed Schedule'}`);
        ...
        if (schedule.schedule_type === 'recurring' && schedule.recurrence_pattern) {
          ...
          if (nextTime) {
            ...
            logToFile(`Calculated next execution time for schedule ${schedule.id}: ${nextTime.toISOString()}`);
          }
        }
        ...
        if (schedule.device_filter) {
          try {
            logToFile(`Applying device filter for schedule ${schedule.id}`);
          ...
          logToFile(`No device filter for schedule ${schedule.id}, fetching all devices`);
          const { data: devices } = await fetchAllDevices(simpleMdmApiKey);
          targetDevices = devices || [];
        }
        
        logToFile(`Applying profile ${schedule.profile_id} to ${targetDevices.length} devices for schedule ${schedule.id}`);
        ...
        logToFile(`Updating schedule ${schedule.id} in database`);
        ...
        const successCount = profileApplicationResults.filter(r => r.success).length;
        const failureCount = profileApplicationResults.filter(r => !r.success).length;
        
        logToFile(`Schedule ${schedule.id} execution completed: ${successCount} successes, ${failureCount} failures`);
        ...
        logToFile(`Error executing schedule ${schedule.id}: ${scheduleError.message}`);
        return {
          scheduleId: schedule.id,
          success: false,
    ...
    logToFile(`Execution completed in ${executionTime}ms: ${cronitorMessage}`);
...
executeSchedules()
  .then(result => {
    logToFile(`Execution complete: ${JSON.stringify(result, null, 2)}`);
    process.exit(0);
  })
  .catch(error => {
